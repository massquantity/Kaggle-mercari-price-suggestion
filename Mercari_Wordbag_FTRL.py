...
class wordbag_itemdes(BaseEstimator, TransformerMixin):
    def __init__(self):
        pass

    def fit(self, X, y=None):
        self.wb = wordbatch.WordBatch(normalize_text, extractor=(WordBag, {"hash_ngrams": 3, "hash_ngrams_weights": [1.0, 1.0, 0.5],
                                                                  "hash_size": 2 ** 29, "norm": "l2", "tf": 1.0,
                                                                  "idf": None}), procs=8)
        self.wb.dictionary_freeze = True
        example = self.wb.fit_transform(X)
        self.mask = np.array(np.clip(example.getnnz(axis=0) - 4, 0, 1), dtype=bool)
        return self

    def transform(self, X):
        wordbatch = self.wb.transform(X)
        wordbatch = wordbatch[:, self.mask]
        print('wordbatch itdes after shape: ', wordbatch.shape)
        print('wordbatch itdes after size: {:.2} mb'.format(sparse_mb(wordbatch)))
        return wordbatch
...
class wordbag_name(BaseEstimator, TransformerMixin):
    def __init__(self):
        pass

    def fit(self, X, y=None):
        self.wb = wordbatch.WordBatch(normalize_text, extractor=(WordBag, {"hash_ngrams": 2, "hash_ngrams_weights": [1.5, 1.0],
                                                                  "hash_size": 2 ** 29, "norm": None, "tf": 'binary',
                                                                  "idf": None,}), procs=8)
        self.wb.dictionary_freeze = True
        example = self.wb.fit_transform(X)
        self.mask = np.array(np.clip(example.getnnz(axis=0) - 1, 0, 1), dtype=bool)
        return self

    def transform(self, X):
        wordbatch = self.wb.transform(X)
        wordbatch = wordbatch[:, self.mask]
        print('wordbatch name after shape: ', wordbatch.shape)
        print('wordbatch name after size: {:.2f} mb'.format(sparse_mb(wordbatch)))
        return wordbatch
...
full_pipe = Pipeline([
    ...
    ('union', FeatureUnion(transformer_list=[
        ...
        ('item_description_tfidf', Pipeline([
            ('selector', select_column(cols="item_description")),
            ('wordbag_itemdes', wordbag_itemdes()),
        ])),
...
start_time  = time.time()
model = FM_FTRL(alpha=0.04, beta=0.01, L1=0.001, L2=0.1, D=train.shape[1], alpha_fm=0.07, L2_fm=0.001, init_fm=0.01,
                    D_fm=400, e_noise=0.0001, iters=2, inv_link="identity", threads=4, weight_fm = 1.0)
model.fit(train,y)
